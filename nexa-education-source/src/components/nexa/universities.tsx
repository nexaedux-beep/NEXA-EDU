...
/** Uniform logo plate: same box for every university, logo centred and scaled to fit. */
function LogoPlate({ uni, className = "h-28 w-full" }: { uni: University; className?: string }) {
  ...
  return (
    <div
      className={`relative shrink-0 overflow-hidden rounded-2xl bg-[var(--logo-plate)] px-5 py-3 ${className}`}
      title={uni.name}
    >
      {exhausted ? (
        <span
          className="nx-gradient-text absolute inset-0 grid place-items-center text-2xl font-extrabold leading-none"
          aria-label={uni.name}
        >
          {initials}
        </span>
        ...
        <img
          ...
          className="absolute inset-0 h-full w-full p-3 object-contain transition-transform duration-500 group-hover:scale-[1.04]"
        />
      )}
    </div>
  );
...
function Card({ uni, index, t }: { uni: University; index: number; t: (k: string) => string }) {
  return (
    <article
      className="nx-card nx-tilt group relative flex flex-col gap-3 overflow-hidden p-4 sm:p-5"
    ...
    >
      ...
      <div className="flex min-w-0 items-center justify-between gap-3 ltr:flex-row-reverse rtl:flex-row">
        <LogoPlate uni={uni} className="h-16 w-16 shrink-0 rounded-full border-2 border-primary/20 bg-card p-1 shadow-inner" />
        <div className="min-w-0 flex-1 ltr:text-left rtl:text-right">
          <h3 className="line-clamp-2 text-[15px] font-bold leading-tight tracking-normal text-foreground">{uni.name}</h3>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 ltr:flex-row-reverse rtl:flex-row">

        <span className="inline-flex items-center gap-1.5 rounded-xl border border-primary/10 bg-accent px-3 py-2 text-[12px] font-bold text-accent-foreground">
          <BadgePercent className="h-4 w-4 shrink-0 text-primary" />
        ...
        <a
          ...
          aria-label={`${t("uni.inquire")} — ${uni.name}`}
          className="nx-btn inline-flex items-center gap-1.5 rounded-xl bg-primary px-3.5 py-2 text-[12px] font-bold text-primary-foreground opacity-90 shadow-[var(--shadow-soft)] group-hover:opacity-100"
        >
          ...
          {[0, 1].map((g) => (
            <div key={g} className="nx-marquee-group" aria-hidden={g === 1}>
              {universities.map((u, i) => (
                <div
                  key={`${u.name}-${i}`}
                  className="group flex h-20 w-[180px] shrink-0 items-center justify-center rounded-2xl border border-border bg-card px-4 transition-all duration-500 hover:-translate-y-1 hover:border-primary/50 hover:shadow-[var(--shadow-soft)]"
                >
                  <LogoPlate uni={u} className="h-14" />
                </div>
              ))}
            </div>
          ))}
