...
const cities: City[] = [
  { name: "إسطنبول", count: 39, x: 156.7, y: 60.3, note: "قلب الحياة الجامعية والمكتب الرئيسي", side: "start" },
  { name: "أنقرة", count: 8, x: 363.6, y: 118.0, note: "العاصمة ومركز الجامعات البحثية", side: "start" },
  { name: "إزمير", count: 3, x: 58.5, y: 198.6, note: "مدينة ساحلية هادئة ومناسبة للدراسة", side: "start" },
  { name: "أنطاليا", count: 2, x: 248.9, y: 279.6, note: "دراسة على البحر المتوسط", side: "start" },
  { name: "قبرص الشمالية", count: 15, x: 390.3, y: 370.8, note: "تكلفة أقل وشهادات معترف بها", side: "end" },
];

export function MapSection() {
  const [activeCity, setActiveCity] = useState<string>("إسطنبول");
  const active = cities.find((c) => c.name === activeCity) ?? cities[0]!;

  return (
    <section id="map" className="border-b border-border bg-background py-14">
          ...
          <div className="space-y-5">
            ...
            <div className="nx-card space-y-2 p-4 text-[13px]">
              <div className="flex items-center gap-2 font-bold">
                <Building2 className="h-4 w-4 text-primary" /> مكتب نيكسا الرئيسي
              </div>
          ...
          <div className="space-y-4">
            <div className="relative overflow-hidden rounded-[28px] border border-border bg-surface p-4 shadow-[var(--shadow-soft)]">
              ...
              <div className="mt-3 flex items-center justify-between gap-3 rounded-2xl border border-border bg-card px-4 py-3">
                <div>
                  <div className="text-[13px] font-extrabold">{active.name}</div>
                  <div className="text-[11px] text-muted-foreground">{active.note}</div>
                </div>
            ...
            <div className="overflow-hidden rounded-[28px] border border-border bg-card shadow-[var(--shadow-soft)]">
              <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
                <div className="flex items-center gap-2 text-[13px] font-bold">
                  <Building2 className="h-4 w-4 text-primary" /> موقع المكتب — إسنيورت، إسطنبول
                </div>
                <a
                  href={site.mapsLink}
                  target="_blank"
              ...
              <iframe
                title="موقع مكتب نيكسا التعليمية"
